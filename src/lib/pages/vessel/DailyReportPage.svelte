<script>
  ...
  function getFuelTotalFromRpmCurveGroups(groups = [], curveType) {
    ...
    return groups.reduce((curveSum, curveGroup) => {
      ...
      const engines = Array.isArray(curveGroup?.engines)
        ? curveGroup.engines
        : [];
      ...
      const engineTotal = engines.reduce((engineSum, engine) => {
        ...
        const fuel = Number(
          totalRow?.fuel_estimated_l ??
            totalRow?.fuelEstimatedL ??
            0
        );
  ...
  function buildTripRouteProjection(points = []) {
    const validPoints = Array.isArray(points)
      ? points.filter(isValidPathPoint)
      : [];
  ...
  function buildDailyTripSummary(data) {
    ...
    const points = Array.isArray(pathCoordinates)
      ? pathCoordinates.filter(isValidPathPoint)
      : [];
    ...
    const totalDistanceFromResponse =
      data?.travel_distance?.total_distance_nm ??
      data?.travelDistance?.totalDistanceNm;
    ...
    return {
      ...
      outsideDistanceNm:
        outsideDistanceFromResponse !== undefined && outsideDistanceFromResponse !== null
          ...
          : calculatePathDistanceNm(
              points,
              (point) => point?.inside_safety_zone === false
            )
    };
  }
  ...
  function getConfiguredEngineNames() {
    const engines =
      Array.isArray(vesselDetail?.engines)
  ...
  function buildOutsideSafetyZoneMap(pathCoordinates = []) {
    ...
    pathCoordinates.forEach((point) => {
      ...
      const insideSafetyZone =
        point?.inside_safety_zone ??
        point?.insideSafetyZone;
  ...
  function normalizeHighRpmOutsideSafetyZoneRows(data) {
    ...
    const highRpmOutside =
      data?.high_rpm_outside_safety_zone ||
      data?.highRpmOutsideSafetyZone ||
      null;
    ...
    if (highRpmOutside && typeof highRpmOutside === "object") {
      ...
      const table15Threshold =
        highRpmOutside?.table_15?.threshold_rpm ??
        highRpmOutside?.table15?.thresholdRpm ??
        "-";

      const table20Threshold =
        highRpmOutside?.table_20?.threshold_rpm ??
        highRpmOutside?.table20?.thresholdRpm ??
        "-";
      ...
      const rows15 = table15Rows.map((row, index) => ({
        ...
        percentageRh: row?.rh_percent ?? row?.rhPercent ?? row?.percentage_rh ?? row?.percentageRh ?? 0,
        avgSpeed: row?.speed_avg ?? row?.speedAvg ?? null,
        highestSpeed: row?.speed_highest ?? row?.speedHighest ?? row?.highest_speed ?? row?.highestSpeed ?? null,
        lowestSpeed: row?.speed_lowest ?? row?.speedLowest ?? row?.lowest_speed ?? row?.lowestSpeed ?? null
      }));
    ...
    const rpmVsFuelChart =
      data?.rpm_vs_fuel_chart ||
      data?.rpmVsFuelChart ||
      {};
    ...
    return rules.flatMap((rule) => {
      return engineNames.map((engineName) => {
        ...
        return {
          ...
          percentageRh:
            runtimeMinutes > 0 ? (durationMinutes / runtimeMinutes) * 100 : 0,
          avgSpeed: null,
          highestSpeed: null,
          lowestSpeed: null
        };
  ...
  function normalizeHighRpmLowSpeed(data) {
    const source =
      data?.high_rpm_low_speed ||
      data?.highRpmLowSpeed ||
      null;
    ...
    const groups = Array.from(groupMap.entries()).map(([engineName, rows]) => {
      ...
      const totalFuel = Number(
        totalFromBackend?.fuel_used_l ??
          totalFromBackend?.fuelUsedL ??
          totalFromRows
      );
    ...
    return {
      ...
      groups,
      totals: filteredTotals.length ? filteredTotals : groups.map((group) => ({
  ...
  function getCurveValueLh(curveEngine, rpm) {
    ...
    const ranges = Array.isArray(curveEngine?.ranges)
      ? curveEngine.ranges
      : [];
    ...
    const valueLh = Number(
      matchedRange?.value_lh ??
        matchedRange?.valueLh
    );
    ...
    return Object.entries(rpmVsFuelChart)
      .map(([engineKey, chartRows]) => {
        ...
        const validRows = downsampleChartRows(chartRows).filter((row) => {
          ...
          const fuelInternal = Number(
            row?.fuel_ems_internal ?? row?.fuelEmsInternal
          );
          const fuelExternal = Number(
            row?.fuel_ems_external ?? row?.fuelEmsExternal
          );
  ...
  let tripMapPoints = $derived(
    getTripMapPoints(normalizedReport)
  );
  ...
  function getRpmRowEngineName(row) {
    ...
    return String(name || "").replace(/\s+TOTAL$/i, "").trim();
  }
    ...
    const tables = [
      {
        ...
        rows: addRpmCurveSource(
          data?.rpm_ranges_ems_internal?.details ||
            data?.rpmRangesEmsInternal?.details,
          "ems_internal"
        )
      },
      {
        ...
        title: "ems_external",
        rows: addRpmCurveSource(
          data?.rpm_ranges_ems_external?.details ||
            data?.rpmRangesEmsExternal?.details,
          "ems_external"
        )
      },
      {
        ...
        title: "Engine Maker",
        rows: addRpmCurveSource(
          data?.rpm_ranges_maker?.details ||
            data?.rpmRangesMaker?.details,
          "engine_maker"
        )
      }
    ];
  ...
  function hasFuelSourceInRpmChartRaw(sourceKey) {
    const chart =
      normalizedReport?.rpm_vs_fuel_chart ||
      normalizedReport?.rpmVsFuelChart ||
      {};
  ...
  let runtimeRows = $derived(
    filterEngineRows(rawRuntimeRows, getRowEngineName)
  );
  ...
  let rpmCurveTables = $derived(
    buildRpmCurveTables(normalizedReport)
  );

  let rpmRows = $derived(
    rpmCurveTables.flatMap((table) => table.rows)
  );
  ...
  let fuelRows = $derived(
    filterEngineRows(rawFuelRows, getRowEngineName)
  );
  ...
  let travelDistance = $derived(
    normalizedReport?.travel_distance ||
      normalizedReport?.travelDistance ||
      {}
  );

  let dailyTripSummary = $derived(
    buildDailyTripSummary(normalizedReport)
  );
  ...
  let highRpmLowSpeedData = $derived(
    normalizeHighRpmLowSpeed(normalizedReport)
  );

  let highRpmLowSpeedGroups = $derived(
    highRpmLowSpeedData.groups || []
  );

  let highRpmLowSpeedTotals = $derived(
    highRpmLowSpeedData.totals || []
  );

  let highRpmLowSpeedGrandTotal = $derived(
    highRpmLowSpeedData.grandTotal || 0
  );

  let rawStatusHistoryGroups = $derived(
    getStatusHistoryGroups(
      normalizedReport?.status_history ||
        normalizedReport?.statusHistory ||
        {}
    )
  );
  ...
  let statusHistoryGroups = $derived(
    sortByConfiguredEngines(
      rawStatusHistoryGroups.filter((group) =>
        isConfiguredEngine(group.engineName)
      ),
      (group) => group.engineName
    )
  );

  let statusTimelineGroups = $derived(
    buildStatusTimelineGroups(statusHistoryGroups)
  );
  ...
  let visibleFuelSourceForDistance = $derived(
    getVisibleFuelSourceLabel()
  );

  let canAccessDailyReport = $derived(
    hasPermission("access_daily_report")
  );

  let canViewEngineRuntimeTable = $derived(
    hasPermission("view_engine_runtime_table")
  );

  let canViewEngineEventStatusHistory = $derived(
    hasPermission("view_engine_event_status_history")
  );

  let canViewEngineOnOffChart = $derived(
    hasPermission("view_engine_on_off_chart")
  );
  ...
  let canViewRpmVsFuelChart = $derived(
    hasPermission("view_rpm_vs_fuel_chart")
  );
  ...
  let visibleRpmCurveTables = $derived(
    rpmCurveTables.filter(canViewRpmCurveTable)
  );

  let visibleRpmRows = $derived(
    visibleRpmCurveTables.flatMap((table) => table.rows || [])
  );

  let canViewRpmRangeRuntimeFuel = $derived(
    visibleRpmCurveTables.length > 0
  );
  ...
  let rpmCurveGroups = $derived(
    buildRpmRangeGroupsByCurve(normalizedReport, engineCurveData)
  );
  ...
  let ecuLiterPerNm = $derived(
    calculateLiterPerNm(ecuFuelValue, totalDistanceNmForFuel)
  );

  let fmsLiterPerNm = $derived(
    calculateLiterPerNm(fmsFuelValue, totalDistanceNmForFuel)
  );

  let totalFuelLiterForDistance = $derived(
    getTotalFuelLiter(fuelSummary)
  );

  let fuelSourceForDistance = $derived(
    getFuelSourceLabel(fuelSummary)
  );
  ...
  let rpmFuelCurveChartGroups = $derived(
    buildRpmFuelCurveChartGroups(normalizedReport)
  );
  ...
  let fodUsage = $derived(
    normalizedReport?.fod_usage ||
      normalizedReport?.fodUsage ||
      {}
  );

  let fodPortValue = $derived(
    fodUsage?.fod_port_l ?? fodUsage?.fodPortL
  );

  let fodStbdValue = $derived(
    fodUsage?.fod_stbd_l ?? fodUsage?.fodStbdL
  );

  let fodSingleValue = $derived(
    fodUsage?.fod_single_l ?? fodUsage?.fodSingleL
  );
  ...
  let hasFodPortStbd = $derived(
    fodPortValue !== undefined ||
      fodStbdValue !== undefined
  );

  let hasFodSingleOnly = $derived(
    !hasFodPortStbd &&
      (fodSingleValue !== undefined && fodSingleValue !== null && fodSingleValue !== "")
  );
  ...
  let canShowFuelEcu = $derived(
    canViewFuelEcu && hasDailyFuelSourceData("ecu")
  );

  let canShowFuelFms = $derived(
    canViewFuelFms && hasDailyFuelSourceData("fms")
  );
  ...
  function updateTopbarStatus(payload) {
    const stats =
      payload?.data_received_stats ||
      payload?.dataReceivedStats ||
      payload?.stats ||
      {};
  ...
  async function handleExportExcel() {
    ...
    try {
      ...
      await downloadApiFile(
        url,
        `Daily_Report_${safeVesselName}_${reportDate}.xlsx`
      );
    } catch (err) {
...
<section class="daily-page">
  <section class="daily-header-card">
    <div>
      ...
      <h1>{vesselName}</h1>
      <p>
        Daily vessel operation report, runtime, speed, RPM range, and fuel usage.
      </p>
    </div>
  </section>
  ...
  <section class="summary-grid">
    ...
    {#if canViewTravelDistanceTable}
      <article class="summary-card">
        ...
        <strong>
          {totalDistanceNmForFuel > 0
            ? `${formatNumber(totalDistanceNmForFuel, 3)} NM`
            : "-"}
        </strong>
      </article>
    {/if}
    ...
    <section class="speed-detail-grid">
      {#if canViewTravelDistanceTable}
        ...
        <article>
          ...
          <strong>
            {travelDistance?.outside_safety_zone_distance_nm || travelDistance?.outsideSafetyZoneDistanceNm
      ...
      {#if canViewDailyPathMap && dailyTripSummary.totalPoints}
        <div class="trip-summary-content">
          <article class="trip-route-card">
            ...
            <div
              class="trip-route-map trip-leaflet-map"
              use:tripLeafletMap={tripMapPoints}
            ></div>
          </article>
            ...
            <div class="trip-summary-list">
                ...
                <div>
                  ...
                  <strong>
                    {literPerNauticalMile === null
                      ? "-"
                      : `${formatNumber(literPerNauticalMile, 2)} L/NM`}
                  </strong>
                </div>
              {/if}
              ...
              {#if canViewLiterPerNauticalMileTable}
                <div>
                  ...
                  <strong>{visibleFuelSourceForDistance}</strong>
                </div>
              {/if}
              ...
              <div>
                ...
                <strong>{dailyTripSummary.totalPoints}</strong>
              </div>
            ...
            <tbody>
              {#each runtimeRows as row}
                <tr>
                  ...
                  <td>{row.runtime_formatted || row.runtimeFormatted || formatHour(row.runtime_hours ?? row.runtimeHours)}</td>
            ...
            <tbody>
              {#each fuelRows as row}
                <tr>
                  ...
                  <td>{formatLiter(getVisibleFuelTotal(row))}</td>
                </tr>
              {/each}
              ...
              <tr class="total-row">
                ...
                <td>{formatLiter(visibleTotalFuelValue)}</td>
              </tr>
            </tbody>
          </table>
        </div>
