<script>
  ...
  let summary = $derived(
    normalizedReport?.summary ||
      normalizedReport?.monthlySummary ||
      normalizedReport?.totals ||
      {}
  );
  ...
  function getDataReceived(row) {
    ...
    if (stats && typeof stats === "object") {
      ...
      const percentage = firstValue(
        stats?.percentage,
        stats?.percent,
        stats?.data_percentage
      );

      if (received !== undefined && total !== undefined) {
        const computedPercent = Number(total)
          ? (Number(received) / Number(total)) * 100
          : 0;

        return `${received} / ${total} (${formatNumber(
    ...
    if (received !== undefined && expected !== undefined) {
      const percent = Number(expected)
        ? (Number(received) / Number(expected)) * 100
        : 0;
  ...
  function isTotalKey(key) {
    return String(key || "").trim().toLowerCase() === "total";
  }
  ...
  function collectEngineKeysFromMonthlyRows(rows = []) {
    ...
    rows.forEach((row) => {
      collectEngineKeysFromObject(
        engineMap,
        row?.runtimes || row?.runtime || {}
      );
  ...
  let monthlyEngines = $derived(
    collectEngineKeysFromMonthlyRows(monthlyRows)
  );
  ...
  let visibleMonthlyEngineFuelSources = $derived(
    monthlyEngineFuelSources.filter((source) =>
      engineFuelSourceHasAnyValue(source.key)
    )
  );
  ...
  let monthlyFodColumns = $derived(
    collectGlobalFuelColumns(monthlyRows, "fod")
  );

  let monthlyBunkerColumns = $derived(
    collectGlobalFuelColumns(monthlyRows, "fuel_bunker")
  );
  ...
  let hasMonthlyFuelColumns = $derived(
    visibleMonthlyEngineFuelSources.length > 0 ||
      visibleMonthlyGlobalFuelGroups.length > 0
  );
  ...
  function getSummaryFuel() {
    const total = monthlyRows.reduce((sum, row) => {
      ...
      const engineFuelTotal = visibleMonthlyEngineFuelSources.reduce(
        (sourceSum, source) => {
      ...
      const globalFuelTotal = visibleMonthlyGlobalFuelGroups.reduce(
        (groupSum, group) => {
          const source = fuelConsumption?.[group.key] || {};

          const hasTotalFod =
            group.key === "fod" &&
            group.columns.some((item) => item.key === "total_fod_l");
          ...
          const columnTotal = group.columns.reduce((columnSum, column) => {
            ...
            const value = getFuelNumber(
              getValueByNormalizedKey(source, column.key)
            );
    ...
    try {
      ...
      const total = rows.reduce((sum, row) => {
        return (
          sum +
          Number(
            row?.data_received_stats?.total_minutes ||
              row?.dataReceivedStats?.totalMinutes ||
              0
          )
        );
      }, 0);
      ...
      setPageStatus({
        dataReceived:
          total > 0 ? `${received} of ${total} (${percentage}%)` : "-",
        queue: payload?.queue ?? "-",
...
<section class="monthly-page">
  <section class="monthly-header-card">
    <div>
      ...
      <h1>{vesselName}</h1>
      <p>
        Monthly operation report by date with runtime, fuel consumption, speed,
        and high RPM low speed summary.
      </p>
    </div>

    <div class="header-meta">
      <span>Vessel ID</span>
      <strong>{$selectedVesselId || "-"}</strong>
    </div>
  </section>
  ...
  <section class="table-section monthly-main-table">
    ...
    {:else}
      <div class="empty-box">
        Monthly report per date belum tersedia.
      </div>
    {/if}
  </section>
