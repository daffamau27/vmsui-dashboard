<script>
  ...
  import {
    getFleetVesselsWithEngines,
    getFleetVesselLiveDetail,
    getFleetAssets
  } from "$lib/api/fleetApi.js";
  ...
  function mergeVesselWithLiveDetail(vessel, liveDetail) {
    if (!vessel) return liveDetail;
    if (!liveDetail) return vessel;

    return {
      ...vessel,
      ...liveDetail,

      // Engine konfigurasi tetap dari /vessels/{id}
      engines: Array.isArray(vessel?.engines) ? vessel.engines : [],

      // Engine RPM live dari /fleet/vessels/{id}
      liveEngines: Array.isArray(liveDetail?.liveEngines)
  ...
  function getLiveEngineRpm(vessel, configEngine) {
    const liveEngines = Array.isArray(vessel?.liveEngines)
      ? vessel.liveEngines
      : [];
  ...
  async function openVesselDashboard(vessel) {
    ...
    try {
      ...
      finalVessel = {
        ...
        // tetap pakai engine konfigurasi dari list awal
        engines: Array.isArray(vessel?.engines) ? vessel.engines : [],

        // simpan juga live rpm engine
        liveEngines: Array.isArray(liveDetail?.liveEngines)
          ? liveDetail.liveEngines
          : []
      };
  ...
  function getEngineRpm(vessel, keyword) {
    ...
    const key = String(keyword || "").toLowerCase().replace(/\s+/g, "_");
    ...
    const engine = engines.find((item) => {
      ...
      return (
        name.includes(key.replace(/_/g, " ")) ||
        engineKey === key ||
        engineKey.includes(key)
      );
    });
  ...
  function getVoyageProgress(vessel) {
    return vessel?.voyageProgress || null;
  }
...
function createAssetPopupHtml(asset) {
  ...
  function refreshMarkerSelection() {
    ...
    markers.forEach((marker, vesselId) => {
      ...
      marker.setZIndexOffset(String(vesselId) === String(selectedVesselId) ? 1000 : 0);
    });
  }
  ...
  async function loadFleetVessels() {
    ...
    try {
      const vessels = await getFleetVesselsWithEngines();

      console.log("[FLEET_VIEW][VESSELS_WITH_ENGINES]", vessels);

      vesselData = vessels;

      const vesselsWithLiveDetail = await loadLiveDetailsForAllVessels(vessels);

      console.log("[FLEET_VIEW][VESSELS_WITH_LIVE_DETAIL]", vesselsWithLiveDetail);

      vesselData = vesselsWithLiveDetail;
  ...
  async function loadLiveDetailsForAllVessels(vessels = []) {
    ...
    const results = await Promise.all(
      vessels.map(async (vessel) => {
        try {
          const liveDetail = await getFleetVesselLiveDetail(
          ...
          <aside class="vessel-detail-panel">
            ...
            <div class="detail-panel-body">
              <section class="detail-section">
                <div class="detail-grid two-col">
                  ...
                  <div class="detail-item">
                    ...
                    <strong class:online-text={selectedVessel.online} class:offline-text={!selectedVessel.online}>
              ...
              <section class="detail-section">
                ...
                <div class="empty-voyage">
                  No voyage plan / progress available for this vessel.
                </div>
              </section>
                ...
                {#if selectedVessel.weather?.current}
                  <div class="weather-current">
                    ...
                    <div class="weather-meta">
                      <strong>{selectedVessel.weather.current.wind_dir || "-"} {selectedVessel.weather.current.wind_speed_kt || "-"} kt</strong>
                      <span>{selectedVessel.weather.current.humidity || "-"}% · {selectedVessel.weather.current.pressure_hpa || "-"} hPa</span>
                    </div>
                  </div>
                {:else}
                ...
                {#if selectedVessel.oceanCurrent?.current}
                  <div class="simple-table">
                    ...
                    <div class="simple-row ocean-observed-row">
                      ...
                      <strong>{formatObservedAt(selectedVessel.oceanCurrent.current.observed_at)}</strong>
                    </div>
                  </div>
                {:else}
...
:global(.asset-popup-title) {
  ...
  background:
    linear-gradient(135deg, rgba(245, 158, 11, 0.16), rgba(245, 158, 11, 0)),
    #fffbeb;
  color: #92400e;
  font-size: 11.5px;
  font-weight: 900;
